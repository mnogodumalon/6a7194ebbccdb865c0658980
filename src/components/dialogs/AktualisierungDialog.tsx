/**
 * AktualisierungDialog — pre-generated create/edit dialog for Aktualisierung.
 *
 ...
 * AktualisierungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Aktualisierung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<AktualisierungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Aktualisierung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Aktualisierung';
...
/** Widened prefill type for AktualisierungDialog.defaultValues — see file header. */
export type AktualisierungDialogDefaults = Omit<Aktualisierung['fields'], 'prioritaet' | 'status'> & {
    prioritaet?: LookupValue | string;
    status?: LookupValue | string;
  };

interface AktualisierungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Aktualisierung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: AktualisierungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  prioritaet: LOOKUP_OPTIONS['aktualisierung']?.['prioritaet'] ?? [],
  status: LOOKUP_OPTIONS['aktualisierung']?.['status'] ?? [],
};
...
export function AktualisierungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: AktualisierungDialogProps) {
  const [fields, setFields] = useState<Partial<Aktualisierung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['titel', 'datum', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Aktualisierung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'aktualisierung');
      await onSubmit(clean as Aktualisierung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "beschreibung": string | null, // Beschreibung\n  "datum": string | null, // YYYY-MM-DD\n  "version": string | null, // Version\n  "prioritaet": LookupValue | null, // Priorität (select one key: "niedrig" | "mittel" | "hoch" | "kritisch") mapping: niedrig=Niedrig, mittel=Mittel, hoch=Hoch, kritisch=Kritisch\n  "status": LookupValue | null, // Status (select one key: "geplant" | "in_bearbeitung" | "abgeschlossen" | "abgebrochen") mapping: geplant=Geplant, in_bearbeitung=In Bearbeitung, abgeschlossen=Abgeschlossen, abgebrochen=Abgebrochen\n  "verantwortliche_person": string | null, // Verantwortliche Person\n  "betroffene_bereiche": string | null, // Betroffene Bereiche\n  "anmerkungen": string | null, // Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Aktualisierung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Aktualisierung bearbeiten' : 'Aktualisierung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. Update 2.5 erfolgreich"
          value={fields.titel ?? ''}
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Änderungen, Verbesserungen, Bugfixes..."
    ...
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">Datum der Aktualisierung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="datum"
          placeholder="Wann erfolgt/erfolgte die Aktualisierung?"
          mode="date"
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'abgebrochen'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'abgebrochen' ? undefined : 'abgebrochen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'abgebrochen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'betroffene_bereiche': (
      <div key="betroffene_bereiche" className="space-y-1.5">
        <Label htmlFor="betroffene_bereiche">Betroffene Bereiche</Label>
        <Input
          id="betroffene_bereiche"
          placeholder="z. B. Frontend, API, Datenbank"
          value={fields.betroffene_bereiche ?? ''}
          onChange={e => setFields(f => ({ ...f, betroffene_bereiche: e.target.value }))}
        />
      </div>
    ),
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="anmerkungen"
          placeholder="Zusätzliche Notizen, bekannte Probleme, Rollback-Plan..."
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "datum": "Datum der Aktualisierung", "version": "Version", "prioritaet": "Priorität", "status": "Status", "verantwortliche_person": "Verantwortliche Person", "betroffene_bereiche": "Betroffene Bereiche", "anmerkungen": "Anmerkungen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.AKTUALISIERUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
