...
import type { Aktualisierung, CreateAktualisierung } from '@/types/app';
...
export class LivingAppsService {
  // --- AKTUALISIERUNG ---
  static async getAktualisierung(): Promise<Aktualisierung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AKTUALISIERUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Aktualisierung[];
    return enrichLookupFields(records, 'aktualisierung');
  }
  static async getAktualisierungEntry(id: string): Promise<Aktualisierung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AKTUALISIERUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Aktualisierung;
    return enrichLookupFields([record], 'aktualisierung')[0];
  }
  static async createAktualisierungEntry(fields: CreateAktualisierung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.AKTUALISIERUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'aktualisierung') });
    return { ...data, record_id: data.id };
  }
  static async updateAktualisierungEntry(id: string, fields: Partial<CreateAktualisierung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.AKTUALISIERUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'aktualisierung') });
    return { ...data, record_id: data.id };
  }
  static async deleteAktualisierungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AKTUALISIERUNG}/records/${id}`);
  }
