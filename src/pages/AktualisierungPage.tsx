...
import { AktualisierungDialog } from '@/components/dialogs/AktualisierungDialog';
...
export default function AktualisierungPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Aktualisierung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Aktualisierung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Aktualisierung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getAktualisierung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Aktualisierung['fields']) {
    await LivingAppsService.createAktualisierungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Aktualisierung['fields']) {
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteAktualisierungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Aktualisierung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Aktualisierung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('verantwortliche_person')}>
                <span className="inline-flex items-center gap-1">
                  Verantwortliche Person
                  {sortKey === 'verantwortliche_person' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('betroffene_bereiche')}>
                <span className="inline-flex items-center gap-1">
                  Betroffene Bereiche
                  {sortKey === 'betroffene_bereiche' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/aktualisierung/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.verantwortliche_person ?? '—'}</TableCell>
                <TableCell>{record.fields.betroffene_bereiche ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={10} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Aktualisierung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
