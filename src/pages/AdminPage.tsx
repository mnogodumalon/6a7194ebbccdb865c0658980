...
import { AktualisierungViewDialog } from '@/components/dialogs/AktualisierungViewDialog';
...
const AKTUALISIERUNG_FIELDS = [
  ...
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }, { key: 'abgebrochen', label: 'Abgebrochen' }] },
  { key: 'verantwortliche_person', label: 'Verantwortliche Person', type: 'string/text' },
  { key: 'betroffene_bereiche', label: 'Betroffene Bereiche', type: 'string/text' },
  { key: 'anmerkungen', label: 'Anmerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'aktualisierung', label: 'Aktualisierung', pascal: 'Aktualisierung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('aktualisierung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'aktualisierung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aktualisierung': return (data as any).aktualisierung as Aktualisierung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aktualisierung': return AKTUALISIERUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aktualisierung': return {
        create: (fields: any) => LivingAppsService.createAktualisierungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAktualisierungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAktualisierungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'aktualisierung' || dialogState?.entity === 'aktualisierung') && (
        <AktualisierungDialog
          open={createEntity === 'aktualisierung' || dialogState?.entity === 'aktualisierung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'aktualisierung' ? handleUpdate : (fields: any) => handleCreate('aktualisierung', fields)}
          defaultValues={dialogState?.entity === 'aktualisierung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Aktualisierung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Aktualisierung']}
        />
      )}
      {viewState?.entity === 'aktualisierung' && (
        <AktualisierungViewDialog
          open={viewState?.entity === 'aktualisierung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'aktualisierung', record: r }); }}
        />
      )}
